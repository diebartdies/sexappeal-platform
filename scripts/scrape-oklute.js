/*
 * scripts/scrape-oklute.js
 *
 * ACCURATE per-site adapter for Oklute (international escort directory with a
 * dedicated Argentina subdomain). Oklute is built on the same platform engine
 * as Skokka, so the markup mirrors scripts/scrape-skokka.js — but Oklute also
 * publishes a canonical WhatsApp deep-link, which is the cleanest contact.
 *
 * WHY THIS EXISTS
 * ----------------
 * Oklute is a global classifieds platform (28 countries). A naive whole-page
 * +54 regex would over-collect and has no country filter. This adapter mirrors
 ...
 *   1. ARGENTINA FILTER = the site's OWN country classification. Oklute puts each
 *      country on its own subdomain; Argentina lives entirely on
 *      https://ar.oklute.com/ , so we never leave it. Each ad ALSO carries a
 *      schema.org PostalAddress with "addressCountry":"AR" and the profile slug
 *      ends in -ar<id>. We assert addressCountry === 'AR' (when present) and
 *      that the contact normalises to an Argentine +54 mobile (guard #2).
 ...
 *   3. REAL CONTACT EXTRACTION = on each /anuncio/ page we read the SPECIFIC
 *      contact element, not the whole page:
 *        - primary:  the WhatsApp deep-link  <a href="https://wa.me/549XXXXXXXXXX">
 *                    which already carries the canonical 549 + 10-digit number
 *                    (most reliable; also covers 3-digit-area-code provinces that
 ...
 * SAFETY
 * ------
 * - DRY RUN by default: writes exports/oklute-leads.csv and touches NO database.
 ...
 * USAGE
 * -----
 *   node scripts/scrape-oklute.js                 # dry run, 10 profiles -> CSV
 *   node scripts/scrape-oklute.js --limit 5       # smaller sample
 *   node scripts/scrape-oklute.js --limit 25 --save   # also upsert to DB
 *   node scripts/scrape-oklute.js --out exports/foo.csv
 *   node scripts/scrape-oklute.js --headful       # watch the browser
 *   node scripts/scrape-oklute.js --proxy user:pass@host:port  # hide your IP
 *   node scripts/scrape-oklute.js --force         # re-fetch everything
 *
 * Flags:
 ...
 *   --out PATH       output CSV path (default exports/oklute-leads.csv)
...
function parseArgs(argv) {
  const args = {
    ...
    listings: 8,
    delayMs: 1500,
    out: path.join('exports', 'oklute-leads.csv'),
    save: false,
    headful: false,
    proxy: null,
    force: false
...
// Oklute shares the same platform engine as Skokka and MAY front pages with a
// Cloudflare "Just a moment..." interstitial. A persistent-profile Chromium
// clears it automatically; poll until the real page appears. Non-fatal.
...
async function extractProfile(page, profileUrl) {
  ...
  try {
    ...
    const data = await page.evaluate((waSel, phoneSel) => {
      ...
      // primary: canonical WhatsApp deep-link (already 549 + national)
      const wa = document.querySelector(waSel);
      out.wa = wa ? wa.getAttribute('href') || '' : '';
      // fallback: schema.org Person JSON-LD (telephone/name/address)
      out.jsonld = '';
    ...
    // city: addressLocality may be "Capital Federal, Centro" -> take the barrio
    if (ldLocality) {
    ...
    // The wa.me form is already "549" + national; the domestic forms start with
    // a "0" trunk or are bare 10-digit. We reject anything carrying a non-54
    // country code, and ALSO require the ad's schema.org addressCountry to be
    // "AR" when present.
    const digits = raw.replace(/[^0-9]/g, '');
    const looksArg =
      digits.startsWith('54') || // wa.me / international +54 form
      digits.startsWith('0') || // domestic 011... form
...
(async () => {
  ...
  console.log(
    `[oklute] start — limit=${args.limit}, listings<=${args.listings}, delay=${args.delayMs}ms, mode=${
  ...
  if (proxy) {
    console.log(`[oklute] routing through proxy ${proxy.server}`);
  } else {
    console.log(
      '[oklute] WARNING: no proxy set — requests go out on your PUBLIC IP. Pass --proxy host:port or set SCRAPE_PROXY to hide it.'
    );
  }
  ...
  if (args.force) {
    if (existing.rows.length) console.log(`[oklute] --force: ignoring ${existing.rows.length} existing rows, re-fetching all`);
  } else {
    for (const p of existing.phones) skipPhones.add(p);
    if (existing.rows.length) {
      console.log(`[oklute] incremental: ${existing.rows.length} already-captured ads will be skipped (use --force to re-fetch all)`);
    }
  }
  ...
  try {
    ...
    browser = await puppeteer.launch({
      ...
      userDataDir: path.join(cacheDir, 'pptr-oklute'),
      args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', ...proxyLaunchArgs(proxy)]
    });
  } catch (e) {
    console.error('[oklute] FAILED to launch Chromium:', e.message);
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`[oklute] opening Argentina home: ${ORIGIN}/`);
    const home = await page.goto(ORIGIN + '/', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await waitForCloudflare(page);
    console.log(`[oklute] home HTTP ${home && home.status()} — ${await page.title()}`);
    await dismissBanners(page);

    // 1) DISCOVERY ----------------------------------------------------------
    const listingUrls = await discoverListingUrls(page, args.listings);
    console.log(`[oklute] discovery across up to ${listingUrls.length} Argentine listing pages`);
    ...
    console.log(`[oklute] discovered ${profileUrls.length} unique Argentine ads; extracting first ${targets.length}`);
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      if (!args.force && entry.phone && skipPhones.has(entry.phone)) {
        console.log(`[oklute] (${i + 1}/${targets.length}) [DUP ] ${entry.alias || '(no alias)'} | already captured — skipping`);
        if (i < targets.length - 1) await sleep(args.delayMs);
        continue;
      }
      const tag = entry.phone ? 'OK ' : 'SKIP';
      console.log(
        `[oklute] (${i + 1}/${targets.length}) [${tag}] ${entry.alias || '(no alias)'} | ${entry.city ||
    ...
    console.error('[oklute] FATAL:', e.message);
  } finally {
  ...
  console.log(`\n[oklute] new this run: ${newRows.length}, total in file: ${totalInFile} -> ${outPath}`);

  // 5) OPTIONAL DB SAVE -----------------------------------------------------
  if (args.save) {
    if (!newRows.length) {
      console.log('[oklute] --save given but no NEW leads to upsert; skipping DB.');
    } else {
      console.log(`[oklute] --save: upserting ${newRows.length} NEW leads into potential_professionals...`);
      try {
        const { inserted, existed } = await saveToDb(newRows);
        console.log(`[oklute] DB done — inserted ${inserted}, already existed ${existed}`);
      } catch (e) {
        console.error('[oklute] DB save failed:', e.message);
        process.exitCode = 1;
      }
    }
  } else {
    console.log('[oklute] DRY RUN — no database connection opened. Re-run with --save to upsert.');
  }
})();
